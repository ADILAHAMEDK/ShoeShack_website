    ...
    <div className={`${DetailsPagePx ? "px-0" : "px-3"} mt-3`}>
        <div className='max-w-[1490px] mx-auto'>
            <div className='sm:flex justify-center items-center sm:justify-between'>
                ...
                <button onClick={()=>handleFilterHomeProducts("all")} className={`${homeFilterBg === "all"? "bg-black text-white" : ""} px-3 py-2 sm:px-3 sm:py-2 border border-black text-black hover:bg-black hover:text-white text-sm sm:text-base font-bold`}>All Brands</button>
                <button onClick={()=>handleFilterHomeProducts("puma")} className={`${homeFilterBg === "PUMA" ? "bg-black text-white" : ""} px-3 py-2 sm:px-3 sm:py-2 border border-black text-black hover:bg-black hover:text-white sm:text-base font-bold`}>Puma</button>
                <button onClick={()=>handleFilterHomeProducts("reebok")} className={`${homeFilterBg === "REEBOK" ? "bg-black text-white" : ""} px-3 py-2 sm:px-3 sm:py-2 border border-black text-black hover:bg-black hover:text-white text-sm sm:text-base font-bold`}>Reebok</button>
                <button onClick={()=>handleFilterHomeProducts("woodLand")} className={`${homeFilterBg === "WOODLAND" ? "bg-black text-white" : ""} px-3 py-2 sm:px-3 sm:py-2 border border-black text-black hover:bg-black hover:text-white text-sm sm:text-base font-bold`}>WoodLand</button>
                </div>
            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 mt-3'>
                {homeFilter && homeFilter.slice(0,5).map((item, index)=>(
                    <div key={index} className='relative hover:border border-black group'>
                        ...
                        {<FaHeart onClick={()=>handleFavorite(item, dispatch)} className={`absolute z-10 top-3 right-2 text-lg ${favorite.some((fav)=> fav.id === item.id )  ? "text-red-700" : "text-black"}`} /> }
                    </div>
                ))}
            </div>
